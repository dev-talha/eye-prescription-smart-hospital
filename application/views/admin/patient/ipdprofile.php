            ...
            <div class="box border0 mb0">
                    <div class="nav-tabs-custom border0 mb0" id="tabs" > 
                        <ul class="nav nav-tabs border-0 navlistscroll" role="tablist">
                            ...
                            <li><a href="#eye_prescription" role="tab" data-toggle="tab" aria-expanded="true"><i class="fa fa-eye"></i> Eye Prescription</a></li>
...
<?php } ?>
                        ...
                        <div class="tab-pane" id="eye_prescription">
                            <div class="box-tab-header">
                                <h3 class="box-tab-title">Eye Prescription</h3>
                                <div class="box-tab-tools">
                                    <a href="<?php echo base_url('admin/eyeprescription/add_ipd/'.$result['patient_id'].'/'.$ipdid); ?>" class="btn btn-primary btn-sm"><i class="fa fa-plus"></i> Add Eye Prescription</a>
                                </div>
                            </div>
                            <div class="timeline-header no-border">
                                <div id="eye_prescription_content">
                                    <div class="text-center"><i class="fa fa-spinner fa-spin"></i> Loading...</div>
                                </div>
                            </div>
                        </div>
                        <script>
                            var eyeRxLoadedIpd = false;
                            $(document).ready(function() {
                                $('a[href="#eye_prescription"]').on('shown.bs.tab', function (e) {
                                    if(!eyeRxLoadedIpd) {
                                        var patient_id = <?php echo $result['patient_id']; ?>;
                                        var ipd_id = <?php echo isset($ipdid) ? $ipdid : "''"; ?>;
                                        $.ajax({
                                            url: base_url + 'admin/eyeprescription/getByIpdAjax',
                                            type: "POST",
                                            data: {patient_id: patient_id, ipd_id: ipd_id},
                                            dataType: 'json',
                                            success: function(res) {
                                                $('#eye_prescription_content').html(res.page);
                                                eyeRxLoadedIpd = true;
                                            }
                                        });
                                    }
